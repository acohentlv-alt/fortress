    ...
    if db["connected"]:
        ...
        try:
            ...
            async with get_conn() as conn:
                ...
                # ── ONE-SHOT CLEANUP: Wipe all scraped data, keep uploads + admin activity ─
                # Requested by Alan on 2026-04-06. Safe to remove this block after first deploy.
                # Guard: only runs while MAPS companies still exist (idempotent after first run).
                _maps_count = await (await conn.execute(
                    "SELECT COUNT(*) FROM companies WHERE siren LIKE 'MAPS%%'"
                )).fetchone()
                if _maps_count and _maps_count[0] > 0:
                    logger.info("ONE-SHOT CLEANUP: starting — %d MAPS companies found", _maps_count[0])

                    # Step 1: Delete scraped contacts on real SIREN companies (keep source='upload')
                    r = await conn.execute(
                        "DELETE FROM contacts WHERE source != 'upload' AND siren NOT LIKE 'MAPS%%'"
                    )
                    logger.info("  Cleanup step 1: deleted %d scraped contacts on real SIRENs", r.rowcount)

                    # Step 2: Delete non-upload officers (keeps source='upload' only)
                    r = await conn.execute(
                        "DELETE FROM officers WHERE source IS NULL OR source != 'upload'"
                    )
                    logger.info("  Cleanup step 2: deleted %d non-upload officers", r.rowcount)

                    # Step 3: Delete all MAPS companies (CASCADE removes their contacts, officers,
                    # batch_tags, batch_log, enrichment_log, company_notes automatically)
                    r = await conn.execute(
                        "DELETE FROM companies WHERE siren LIKE 'MAPS%%'"
                    )
                    logger.info("  Cleanup step 3: deleted %d MAPS companies (CASCADE applied)", r.rowcount)

                    # Step 4: Delete discovery batch_data rows (keep mode='upload')
                    r = await conn.execute(
                        "DELETE FROM batch_data WHERE mode IS NULL OR mode != 'upload'"
                    )
                    logger.info("  Cleanup step 4: deleted %d non-upload batch_data rows", r.rowcount)

                    # Step 5: Delete batch_log entries orphaned after step 4 (no parent in batch_data)
                    r = await conn.execute(
                        "DELETE FROM batch_log WHERE batch_id NOT IN (SELECT batch_id FROM batch_data)"
                    )
                    logger.info("  Cleanup step 5: deleted %d orphaned batch_log entries", r.rowcount)

                    # Step 6: Delete batch_tags not belonging to any remaining upload batch
                    r = await conn.execute("""
                        DELETE FROM batch_tags
                        WHERE batch_name IS NULL
                           OR batch_name NOT IN (
                               SELECT batch_name FROM batch_data WHERE batch_name IS NOT NULL
                           )
                    """)
                    logger.info("  Cleanup step 6: deleted %d orphaned batch_tags rows", r.rowcount)

                    # Step 7: Clear ancillary tables
                    r = await conn.execute("DELETE FROM query_memory")
                    logger.info("  Cleanup step 7a: deleted %d query_memory rows", r.rowcount)
                    r = await conn.execute("DELETE FROM blacklisted_sirens")
                    logger.info("  Cleanup step 7b: deleted %d blacklisted_sirens rows", r.rowcount)
                    r = await conn.execute("DELETE FROM enrichment_log")
                    logger.info("  Cleanup step 7c: deleted %d enrichment_log rows", r.rowcount)

                    # Step 8: Clear activity log except admin (username='alan')
                    r = await conn.execute(
                        "DELETE FROM activity_log WHERE username != 'alan'"
                    )
                    logger.info("  Cleanup step 8: deleted %d non-admin activity log rows", r.rowcount)

                    # Step 9: Reset MAPS ID sequence to start from 1 on next batch
                    await conn.execute("ALTER SEQUENCE maps_id_seq RESTART WITH 1")
                    logger.info("  Cleanup step 9: maps_id_seq reset to 1")

                    logger.info("ONE-SHOT CLEANUP: complete — scraped data wiped, uploads + admin log preserved")
                else:
                    logger.info("ONE-SHOT CLEANUP: skipped — no MAPS companies found (already ran)")
                # ── END ONE-SHOT CLEANUP ─────────────────────────────────────────────────
