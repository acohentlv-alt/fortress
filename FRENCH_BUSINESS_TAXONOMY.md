...
- **Civic-title abbreviation expansion (May 8 — `entities._STREET_ABBREVS`):** French government registries (INPI, SIRENE) routinely abbreviate honorifics in street names. The abbreviation table at `_STREET_ABBREVS` (in `fortress/matching/entities.py`) expands these bidirectionally before address-key comparison: `mal→marechal`, `gal→general`, `gen→general`, `col→colonel`, `cdt→commandant`, `lt→lieutenant`, `dr→docteur`, `st→saint`, `ste→sainte`. Phase 4 INPI corroboration is the primary beneficiary. **Explicitly rejected expansions** (do not add): `mar→marechal` (Mediterranean lagoon toponym Mar Estang, Mar Vivo — 50+ rows in coastal Languedoc), `cap→capitaine` (Atlantic coastal toponyms Cap-Ferret, Cap d'Agde — Maps embeds them in street names, postal-code stripping does not catch them), `pr→professeur` (Catalan-Spanish noise in 66/64 dept addresses, ~1 Maps row volume), `me→maitre` (130 SIRENE rows — too low yield to justify ambiguity). The Phase A `_verify_signals` and Step 4b surname S5 paths use bare `_extract_street_key` (no expansion) — separate gaps, NOT covered by this table; bridging them requires weighing pre-confirm gate strictness against rescue lift.
